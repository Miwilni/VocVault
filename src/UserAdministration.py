...
class User:
    ...
    def db_get_username(self):
        ...
        exists: int = execute_get_query(f"SELECT EXISTS (SELECT 1 FROM `User` WHERE Username = '{self.__username}');")[0][0]
        if exists == 0:
    ...
    #     self.__foreign_languages = execute_get_query(f"SELECT ForeignLanguages FROM User WHERE Username = '{self.__username}'")[0][0] #TODO: Change
